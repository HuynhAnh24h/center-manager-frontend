...
function ExportMenu({ getFilters }) {
  ...
  const doExport = async (extraParams, filename) => {
    ...
    try {
      const rows = (await fetchAll({ ...getFilters(), ...extraParams })).map(
        toRow,
      );
      if (!rows.length) {
  ...
  const items = [
    ...
    },
    {
      ...
      file: "HocPhi_DongMotPhan",
    },
    null,
    {
      label: "Xuất theo bộ lọc hiện tại",
      params: {},
      file: "HocPhi_LocHienTai",
    },
    {
      label: "Xuất tất cả",
          ...
          >
            ...
            {items.map((item, i) =>
              !item ? (
                ...
                <button
                  key={i}
                  onClick={() => doExport(item.params, item.file)}
                  style={{
                    display: "flex",
                    alignItems: "start",
                    gap: 10,
                  ...
                  onMouseEnter={(e) =>
                    (e.currentTarget.style.background = "var(--gold-50)")
                  }
                  onMouseLeave={(e) =>
...
export default function TuitionPage() {
  ...
  const clearAllFilters = () => {
    ...
    setPmFilter("");
    dispatch(setPage(1));
    dispatch(fetchTuitions({ page: 1, limit: 15 }));
  };

  const toggleStatus = (key) => {
    dispatch(setStatusFilter(statusFilter === key ? "" : key));
    dispatch(setPage(1));
  };
  ...
  const totalPending =
    (statMap["pending"]?.totalAmount || 0) +
    (statMap["overdue"]?.totalAmount || 0) +
    (statMap["partial"]?.totalAmount || 0);
  const pctPaid =
  ...
  const chips = [
    ...
    statusFilter && {
      label: `Trạng thái: ${STATUS_TUITION[statusFilter]?.label}`,
      clear: () => dispatch(setStatusFilter("")),
    },
    classFilter && {
      label: `Lớp: ${classes.find((c) => c._id === classFilter)?.className || "..."}`,
      clear: () => setClassFilter(""),
    },
    courseFilter && {
      label: `Khóa: ${courses.find((c) => c._id === courseFilter)?.courseName || "..."}`,
      clear: () => setCourseFilter(""),
    },
    (monthFilter || yearFilter) && {
      label: `Tháng ${monthFilter || "?"}/${yearFilter || "?"}`,
      clear: () => {
        setMonthFilter("");
        setYearFilter("");
      },
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (result.error) {
        toast.error(result.payload || "Lỗi tạo phiếu");
        return;
      }
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      if (result.error) {
        toast.error(result.payload || "Lỗi cập nhật");
        return;
      }
  ...
  return (
    <>
      <Topbar
        ...
        subtitle="Theo dõi tình trạng đóng học phí"
      />
      <div className="page-body">
        ...
        >
          {[
            ...
            {
              special: true,
              label: `Doanh thu T${new Date().getMonth() + 1}`,
            ...
            <div
              ...
              style={{
                background: s.bg,
                border: `2px solid ${statusFilter === s.key ? s.color : s.border}`,
                borderRadius: 12,
                padding: "14px 16px",
                cursor: s.key ? "pointer" : "default",
                transition: "all 0.15s",
                boxShadow:
                  statusFilter === s.key ? `0 0 0 3px ${s.color}22` : "none",
              }}
          ...
          >
            ...
            >
              ...
              <span style={{ color: "var(--text-muted)" }}>
                ...
                <strong style={{ color: "#059669", marginLeft: 8 }}>
                  ({pctPaid}%)
                </strong>
              </span>
            </div>
        ...
        >
          <div
            style={{ position: "relative", flex: "1 1 260px", minWidth: 220 }}
          >
            <Search
              size={14}
              style={{
            ...
            <input
              ...
              style={{ paddingLeft: 36, paddingRight: localSearch ? 34 : 12 }}
              value={localSearch}
          ...
          >
            ...
            {Object.entries(STATUS_TUITION).map(([k, v]) => (
          ...
          >
            ...
            {chips.filter(
              (c) =>
                !String(c.label).startsWith("🔍") &&
                !String(c.label).startsWith("Trạng"),
            ).length > 0 && (
              <span
                style={{
        ...
        {chips.length > 0 && (
          ...
          >
            ...
            {chips.map((c, i) => (
              <Chip key={i} label={c.label} onRemove={c.clear} />
            ))}
                    ...
                    >
                      ...
                      <td>
                        ...
                        >
                          ...
                          <div>
                            ...
                            >
                              {t.student?.studentCode} · {t.student?.phone}
                            </div>
                          </div>
                        </div>
                      </td>
                      ...
                      <td
                        style={{
                          ...
                          fontSize: 13,
                          color:
                            t.discount > 0 ? "#059669" : "var(--text-muted)",
                        }}
                      >
                        {t.discount > 0
            ...
            >
              ...
              <span>
                ...
                <strong style={{ color: "#059669" }}>
                  {formatCurrency(pagePaid)}
                </strong>
              </span>
              <span>
                Chưa thu:{" "}
                <strong style={{ color: "#d97706" }}>
                  {formatCurrency(pageUnpaid)}
                </strong>
              </span>
            </div>
          )}
            ...
            <>
              ...
              {["pending", "overdue", "partial"].includes(selected.status) && (
                <button
                  ...
                  onClick={() => {
                    setModal(null);
                    setTimeout(() => openPay(selected), 50);
                  }}
                >
        ...
        >
          <div
            style={{
              textAlign: "center",
              padding: "12px 0 18px",
              borderBottom: "2px dashed var(--gold-200)",
              marginBottom: 16,
            }}
          >
            <div
            ...
            <Badge
              {...(STATUS_TUITION[selected.status] || {
            ...
            {[
              ...
              [
                ...
                selected.paymentMethod
                  ? `${PM_ICON[selected.paymentMethod]} ${PAYMENT_METHODS[selected.paymentMethod]}`
                  : "—",
              ],
            ].map(([label, value]) => (
              <div
          ...
          >
            ...
            <div
              style={{
                ...
                justifyContent: "space-between",
                paddingTop: 8,
                borderTop: "1px dashed var(--gold-300)",
                fontSize: 16,
              }}
            >
              <span style={{ fontWeight: 700, color: "var(--gold-800)" }}>
                Thực thu
              </span>
              <span style={{ fontWeight: 900, color: "var(--gold-800)" }}>
            ...
            >
              ...
              {students.map((s) => (
                <option key={s._id} value={s._id}>
                  {s.studentCode} — {s.fullName} ({s.phone})
                </option>
              ))}
            </select>
          </div>
          <FormRow cols={2}>
            ...
            <div className="form-group">
              ...
              >
                ...
                {courses.map((c) => (
                  <option key={c._id} value={c._id}>
                    {c.courseName} — {formatCurrency(c.fee)}
                  </option>
                ))}
              </select>
            </div>
