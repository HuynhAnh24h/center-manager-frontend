...
import { UserPlus, Pencil, Trash2, Eye, CreditCard, X, CheckCircle, Clock } from 'lucide-react';
...
export default function StudentsPage() {
  ...
  const { list: students, total, totalPages, page, search, statusFilter, loading } = useSelector(s => s.students);
  ...
  // Tuition state (local — fetched per student on demand)
  const [tuitions, setTuitions] = useState([]);
  const [tuitionSummary, setTuitionSummary] = useState(null);
  const [tuitionLoading, setTuitionLoading] = useState(false);

  const load = useCallback(() => {
    dispatch(fetchStudents({ page, limit: 10, search, status: statusFilter || undefined }));
  }, [dispatch, page, search, statusFilter]);

  useEffect(() => { load(); }, [load]);

  const debouncedSearch = useCallback(
    debounce((val) => dispatch(setSearch(val)), 400), []
  );
  ...
  const openTuition = async (s) => {
    setSelected(s); setModal('tuition');
    setTuitionLoading(true); setTuitions([]); setTuitionSummary(null);
    try {
      const { data } = await tuitionService.getByStudent(s._id);
      setTuitions(data.data); setTuitionSummary(data.summary);
  ...
  const handleSave = async () => {
    ...
    try {
      let result;
      if (modal === 'create') {
        result = await dispatch(createStudent(form));
      } else {
        result = await dispatch(updateStudent({ id: selected._id, data: form }));
      }
      if (result.error) { toast.error(result.payload || 'Có lỗi xảy ra'); return; }
      toast.success(modal === 'create' ? 'Thêm học viên thành công!' : 'Cập nhật thành công!');
      setModal(null);
    } finally { setSaving(false); }
  };

  const handleDelete = async () => {
    const result = await dispatch(deleteStudent({ id: selected._id, name: selected.fullName }));
  ...
  return (
    <>
      ...
      <div className="page-body">
        <PageHeader title="Danh sách học viên"
          actions={<button className="btn btn-primary" onClick={openCreate}><UserPlus size={15} /> Thêm học viên</button>}
        />
        ...
        <div className="card">
          <div className="table-wrapper">
            <table>
              <thead>
                <tr>
                  <th>Mã HV</th><th>Họ và tên</th><th>Điện thoại</th><th>Giới tính</th>
                  <th>Phụ huynh</th><th>Trạng thái</th><th>Ngày đăng ký</th>
                  <th style={{ textAlign: 'right' }}>Thao tác</th>
                </tr>
              </thead>
              <tbody>
                {loading ? <LoadingRows cols={8} rows={8} />
                  : students.length === 0 ? <tr><td colSpan={8}><EmptyState message="Chưa có học viên nào" /></td></tr>
                  : students.map(s => (
                    <tr key={s._id}>
      ...
      {(modal === 'create' || modal === 'edit') && (
        <Modal title={modal === 'create' ? 'Thêm học viên mới' : 'Chỉnh sửa học viên'}
          ...
          <FormRow cols={2}>
            <div className="form-group"><label className="form-label">Họ và tên *</label><input className="form-control" value={form.fullName} onChange={F('fullName')} placeholder="Nguyễn Văn A" /></div>
            <div className="form-group"><label className="form-label">Giới tính</label><select className="form-control" value={form.gender} onChange={F('gender')}><option value="male">Nam</option><option value="female">Nữ</option><option value="other">Khác</option></select></div>
          </FormRow>
          <FormRow cols={2}>
            <div className="form-group"><label className="form-label">Số điện thoại</label><input className="form-control" value={form.phone} onChange={F('phone')} placeholder="0901234567" /></div>
            <div className="form-group"><label className="form-label">Email</label><input className="form-control" type="email" value={form.email} onChange={F('email')} placeholder="email@gmail.com" /></div>
          </FormRow>
          <FormRow cols={2}>
            <div className="form-group"><label className="form-label">Ngày sinh</label><input className="form-control" type="date" value={form.dateOfBirth} onChange={F('dateOfBirth')} /></div>
            <div className="form-group"><label className="form-label">Trạng thái</label><select className="form-control" value={form.status || 'active'} onChange={F('status')}>{Object.entries(STATUS_STUDENT).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></div>
          </FormRow>
          <div className="form-group"><label className="form-label">Địa chỉ</label><input className="form-control" value={form.address} onChange={F('address')} placeholder="Số nhà, đường, quận..." /></div>
          <FormRow cols={2}>
            <div className="form-group"><label className="form-label">Tên phụ huynh</label><input className="form-control" value={form.parentName} onChange={F('parentName')} placeholder="Nguyễn Văn B" /></div>
            <div className="form-group"><label className="form-label">SĐT phụ huynh</label><input className="form-control" value={form.parentPhone} onChange={F('parentPhone')} placeholder="0912345678" /></div>
          </FormRow>
          <div className="form-group"><label className="form-label">Ghi chú</label><textarea className="form-control" rows={2} value={form.notes} onChange={F('notes')} placeholder="Thông tin thêm..." /></div>
        </Modal>
      )}
      ...
      {modal === 'detail' && selected && (
        <Modal title="Chi tiết học viên" onClose={() => setModal(null)} size="md"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 20px' }}>
            {[
              ['Mã học viên', selected.studentCode], ['Họ và tên', selected.fullName],
              ['Giới tính', selected.gender === 'male' ? 'Nam' : selected.gender === 'female' ? 'Nữ' : 'Khác'],
              ['Điện thoại', selected.phone || '—'], ['Email', selected.email || '—'],
              ['Ngày sinh', formatDate(selected.dateOfBirth)],
              ['Phụ huynh', selected.parentName || '—'], ['SĐT PH', selected.parentPhone || '—'],
            ].map(([label, value]) => (
          ...
          <div style={{ marginTop: 12 }}><div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 6 }}>Trạng thái</div><Badge {...STATUS_STUDENT[selected.status]} /></div>
          {selected.notes && <div style={{ marginTop: 12, padding: 12, background: 'var(--gold-50)', borderRadius: 8, border: '1px solid var(--gold-200)', fontSize: 13 }}>{selected.notes}</div>}
        </Modal>
      )}
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(null)}>
          <div className="modal" style={{ maxWidth: 780, width: '100%' }}>
            <div className="modal-header">
              <div><h3 className="modal-title">Học phí — {selected.fullName}</h3><p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 3 }}>{selected.studentCode}</p></div>
              <button className="btn btn-ghost btn-icon" onClick={() => setModal(null)}><X size={18} /></button>
            </div>
            <div className="modal-body">
              {tuitionLoading ? <div style={{ padding: 48, textAlign: 'center', color: 'var(--text-muted)' }}>Đang tải...</div> : (
                <>
                  {tuitionSummary && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
                      ...
                      <div style={{ padding: '14px 16px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
                        ...
                        <div style={{ fontSize: 11, color: '#166534', marginTop: 2 }}>{tuitionSummary.totalAmount > 0 ? Math.round(tuitionSummary.paidAmount / tuitionSummary.totalAmount * 100) : 0}% tổng học phí</div>
                      </div>
                      <div style={{ padding: '14px 16px', borderRadius: 10, background: '#fff7ed', border: '1px solid #fed7aa' }}>
                        ...
                        <div style={{ fontSize: 11, color: '#9a3412', marginTop: 2 }}>{tuitionSummary.totalAmount > 0 ? Math.round(tuitionSummary.unpaidAmount / tuitionSummary.totalAmount * 100) : 0}% tổng học phí</div>
                      </div>
                    </div>
                  )}
                  {tuitionSummary?.totalAmount > 0 && (
                    <div style={{ marginBottom: 20 }}>
                      ...
                      <div style={{ height: 8, background: 'var(--gray-100)', borderRadius: 8, overflow: 'hidden' }}>
                        <div style={{ height: '100%', width: `${Math.min(100, Math.round(tuitionSummary.paidAmount / tuitionSummary.totalAmount * 100))}%`, background: 'linear-gradient(90deg, #16a34a, #22c55e)', borderRadius: 8 }} />
                      </div>
                    </div>
                  )}
                  {tuitions.length === 0 ? <EmptyState message="Chưa có phiếu học phí nào" /> : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {tuitions.map(t => (
                        <div key={t._id} style={{ padding: '12px 14px', border: '1px solid var(--border)', borderRadius: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ fontWeight: 600, fontSize: 13 }}>{t.class?.className || '—'}</div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{t.course?.courseName} · Hạn: {formatDate(t.dueDate)}{t.paidDate && ` · Đã đóng: ${formatDate(t.paidDate)}`}</div>
                          </div>
                          ...
                          <div style={{ flexShrink: 0 }}><Badge {...(STATUS_TUITION[t.status] || { label: t.status, cls: 'badge-gray' })} /></div>
                        </div>
                      ))}
                    </div>
                  )}
      ...
      {modal === 'delete' && selected && (
        <ConfirmModal title="Xóa học viên"
