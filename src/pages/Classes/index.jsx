...
import { formatDate, formatCurrency, STATUS_CLASS, SHIFT_LABEL, DAY_LABEL, LEVEL_LABEL, debounce } from '../../utils/helpers.js';
...
export default function ClassesPage() {
  ...
  const fetchClasses = useCallback(async (p = 1, s = '', st = '', sh = '', tp = '') => {
    setLoading(true);
    try {
      const { data } = await classService.getAll({
        page: p, limit: 10, search: s,
        status: st || undefined, shift: sh || undefined,
        classType: tp || undefined,
      });
  ...
  useEffect(() => {
    fetchClasses(1, search, statusFilter, shiftFilter, typeFilter);
    courseService.getAll({ limit: 100 }).then(({ data }) => setCourses(data.data)).catch(() => {});
  }, [statusFilter, shiftFilter, typeFilter]);

  useEffect(() => { fetchClasses(page, search, statusFilter, shiftFilter, typeFilter); }, [page]);

  const debouncedSearch = useCallback(
    debounce((v) => { setPage(1); fetchClasses(1, v, statusFilter, shiftFilter, typeFilter); }, 400),
    [statusFilter, shiftFilter, typeFilter],
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setModal(null); fetchClasses(page, search, statusFilter, shiftFilter, typeFilter);
    } catch (e) { toast.error(e.response?.data?.message || 'Lỗi'); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    try { await classService.delete(selected._id); toast.success('Đã xóa lớp học'); setModal(null); fetchClasses(1, search, statusFilter, shiftFilter, typeFilter); }
  ...
  const refreshClassDetail = async () => {
    ...
    fetchClasses(page, search, statusFilter, shiftFilter, typeFilter);
  };
    ...
    <>
      ...
      <div className="page-body">
        ...
        <div className="flex items-center gap-3 mb-4" style={{ flexWrap: 'wrap' }}>
          ...
          <select className="form-control" style={{ width: 150 }} value={typeFilter} onChange={(e) => { setTypeFilter(e.target.value); setPage(1); }}>
        ...
        <div className="card">
          <div className="table-wrapper">
            <table>
              ...
              <tbody>
                {loading ? <LoadingRows cols={9} rows={6} /> : classes.length === 0 ? (
                  <tr><td colSpan={9}><EmptyState message="Chưa có lớp học nào" /></td></tr>
                ) : classes.map((c) => (
                  <tr key={c._id}>
                    ...
                    <td>
                      {c.classType && (
                        <span style={{ background: CLASS_TYPE_COLOR[c.classType]?.bg || '#f3f4f6', color: CLASS_TYPE_COLOR[c.classType]?.text || '#374151', padding: '2px 10px', borderRadius: 10, fontSize: 11, fontWeight: 600 }}>
                    ...
                    <td>
                      ...
                      {c.course?.description && (
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          {c.course.description}
                        </div>
                      )}
                      {c.course?.level && (
                        <span style={{ fontSize: 10, background: 'var(--gold-100)', color: 'var(--gold-800)', padding: '1px 6px', borderRadius: 8, fontWeight: 600 }}>
        ...
        <Modal title={modal === 'create' ? 'Tạo lớp học mới' : 'Chỉnh sửa lớp học'} onClose={() => setModal(null)} size="lg"
          ...
          <FormRow cols={2}>
            ...
            <div className="form-group">
              ...
              <select className="form-control" value={form.classType} onChange={F('classType')}>
                {Object.entries(CLASS_TYPE_LABEL).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
          </FormRow>
            ...
            <select className="form-control" value={form.course} onChange={F('course')}>
              ...
              {courses.map((c) => (
                <option key={c._id} value={c._id}>
                  {c.courseName}{c.level ? ` (${LEVEL_LABEL[c.level] || c.level})` : ''}{c.fee ? ` — ${new Intl.NumberFormat('vi-VN').format(c.fee)}đ` : ''}
                </option>
              ))}
            </select>
            {selectedCourse?.description && (
              <div style={{ marginTop: 6, padding: '8px 12px', background: 'var(--gold-50)', borderRadius: 8, border: '1px solid var(--gold-200)', fontSize: 12, color: 'var(--text-secondary)' }}>
                <strong>Mô tả:</strong> {selectedCourse.description}
                {selectedCourse.duration && <span style={{ marginLeft: 8 }}>· <strong>{selectedCourse.duration} buổi</strong></span>}
              </div>
            )}
          </div>
          <FormRow cols={3}>
            ...
            <div className="modal-header">
              <div>
                ...
                <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 3 }}>
                  {selected.classCode} ·{' '}
                  {classDetail ? `${classDetail.students?.length || 0} / ${classDetail.maxStudents} chỗ${isFull ? ' — Lớp đã đầy' : ''}` : '...'}
                </p>
              </div>
                  ...
                  <div>
                    ...
                    <div style={{ maxHeight: 400, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {!classDetail?.students?.length ? (
                        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)', fontSize: 13 }}><Users size={32} style={{ margin: '0 auto 8px', opacity: 0.25 }} />Chưa có học viên nào</div>
                  ...
                  <div>
                    ...
                    <div style={{ maxHeight: 400, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {availableStudents.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)', fontSize: 13 }}><Search size={28} style={{ margin: '0 auto 8px', opacity: 0.2 }} />{studentSearch ? 'Không tìm thấy học viên phù hợp' : 'Tất cả học viên đã trong lớp'}</div>
